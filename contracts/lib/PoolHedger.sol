...
abstract contract PoolHedger {
   ...
   */
  function getCurrentHedgedNetDelta() external virtual view returns (int);
  ...
  function getHedgingLiquidity(uint spotPrice) external virtual view returns (uint pendingDeltaLiquidity, uint usedDeltaLiquidity);
   ...
   */
  function getCappedExpectedHedge() public virtual view returns (int cappedExpectedHedge);
  ...
  function getPoolHedgerParams() external virtual view returns (PoolHedgerParameters memory) {
