...
export async function getLPExposure(params: Params, address: string, tickers: string[]) {
  ...
  for (const ticker of tickers) {
    ...
    const totalValue = decimalToBN(latestRoundStartedEvent.totalTokenSupply);

    let totalValueForAddress = BigNumber.from(0);

    for (const certificateId of certificates) {
      const certData = await callLyraFunction(
        params,
    ...
    for (const boardId of liveBoards) {
      ...
      for (const listing of listings) {
        deltaExposurePerListing[listing.listingId.toNumber()] = {
          listingDeltaExposure: listing.longCall
            .mul(listing.callDelta)
            .div(UNIT)
            .sub(listing.shortCall.mul(listing.callDelta).div(UNIT))
            .add(listing.longPut.mul(listing.putDelta).div(UNIT))
            .sub(listing.shortPut.mul(listing.putDelta).div(UNIT)),
    ...
    const netDelta: any = Object.values(deltaExposurePerListing).reduce(
      (a: any, b: any) => a.add(b.listingDeltaExposure),
      BigNumber.from(0),
    );
    const collateralExposure: any = Object.values(deltaExposurePerListing).reduce(
      (a: any, b: any) => a.add(b.collateralExposure),
      BigNumber.from(0),
    );

    const cacheNetDelta: BigNumber = await callLyraFunction(
      params,
      'OptionGreekCache',
      'getGlobalNetDelta',
      [],
    ...
    const baseBalance: BigNumber = await callSynthetixFunction(params, 'Proxy' + ticker, 'balanceOf', [
      (await getLyraContract(params, 'LiquidityPool', ticker)).address,
    ]);
