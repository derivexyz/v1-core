  ...
  ALL_TYPES.forEach(async optionType => {
    describe('trade: ' + OptionType[optionType], async () => {
      ...
      it('allows opening more of 0 amount', async () => {
        ...
        const ethprice = await hre.f.c.synthetixAdapter.getSpotPriceForMarket(hre.f.c.optionMarket.address, 2);
        await expectLiquidityFunds(optionType, toBN('1'), hre.f.strike.strikeId, ethprice);
        if (optionType == OptionType.SHORT_CALL_BASE) {
...
export async function expectLiquidityFunds(
  optionType: OptionType,
  openSize: BigNumber,
  strikeId: BigNumberish,
  ethOpenPrice: BigNumber,
) {
  ...
  if (optionType == OptionType.LONG_CALL) {
    expect(liquidity.reservedCollatLiquidity).to.eq(
      ethOpenPrice.mul(openSize).mul(lpParams.callCollatScalingFactor).div(UNIT).div(UNIT),
    );
  } else if (optionType == OptionType.LONG_PUT) {
    expect(liquidity.reservedCollatLiquidity).to.eq(
      strikePrice.mul(openSize).mul(lpParams.putCollatScalingFactor).div(UNIT).div(UNIT),
    );
  }
}
