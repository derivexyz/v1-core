...
describe('USDC_quote - OptionMarket - SM Claim', () => {
  ...
  it('can harvest all base if accidentally donated', async () => {
    await hre.f.c.snx.baseAsset.mint(hre.f.c.optionMarket.address, 1e8);
    const oldSMBal = await hre.f.c.snx.baseAsset.balanceOf(sm.address);
    expect(await hre.f.c.snx.baseAsset.balanceOf(hre.f.c.optionMarket.address)).eq(1e8);
    ...
    expect(oldSMBal.add(1e8)).to.eq(await hre.f.c.snx.baseAsset.balanceOf(sm.address));
  });

  it('claim 0 amounts', async () => {
    await openAllTrades();
    await hre.f.c.snx.baseAsset.transfer(hre.f.c.optionMarket.address, 1e8);
    await hre.f.c.optionMarket.connect(sm).smClaim();
  ...
  it('claims both balances', async () => {
    ...
    expect(oldSMQuoteBal.add(1000e6)).to.eq(await hre.f.c.snx.quoteAsset.balanceOf(sm.address));
    expect(oldSMBaseBal.add(1e8)).to.eq(await hre.f.c.snx.baseAsset.balanceOf(sm.address));
  });
});
