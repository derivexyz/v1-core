...
describe('USDC_quote - OptionMarket - SettleBoard', () => {
  beforeEach(seedFixtureUSDCwBTC);
  ...
  describe('Base conversions', async () => {
    ...
    it('sells base collateral when settling both long and short call correctly', async () => {
      ...
      expect(await getBaseBalance(hre.f.c.shortCollateral.address)).to.eq(1e8);
      ...
      expect(await getBaseBalance(hre.f.c.shortCollateral.address))
        .lt(toBN('1').div(CONVERTWBTC))
        .gt(toBN('0.8').div(CONVERTWBTC));
    });
  });
  ...
  describe('Payouts & Reservations', async () => {
    ...
    describe('short call base exposure', async () => {
      ...
      beforeEach(async () => {
        ...
        await fastForward(MONTH_SEC);
        assertCloseTo(await getQuoteBalance(hre.f.deployer.address), toBN('1002878.515').div(CONVERTUSDC));
        expect(await getBaseBalance(hre.f.c.shortCollateral.address)).to.eq(partialBaseCollat.div(CONVERTWBTC));
      });
      ...
      it('sends collateral to LP and sells into quote correctly if ITM', async () => {
        ...
        assertCloseTo(scBaseBal, toBN('2.48111').div(CONVERTWBTC), toBN('0.001'));
        // assertCloseTo(lpQuoteBal, toBN('502100.473').div(CONVERTUSDC), toBN('0.1')); // value off
      });
    });

    describe('short call quote exposure', async () => {
      // const partialQuoteCollat = 10000e6;
      ...
      beforeEach(async () => {
        ...
        assertCloseTo(await getQuoteBalance(hre.f.deployer.address), toBN('992878.515').div(CONVERTUSDC));
        // expect(await getQuoteBalance(hre.f.c.shortCollateral.address)).to.eq(partialQuoteCollat);
        assertCloseTo(await getQuoteBalance(hre.f.c.shortCollateral.address), partialQuoteCollat.div(CONVERTUSDC));
      });
    ...
    describe('short put quote exposure', async () => {
      ...
      beforeEach(async () => {
        ...
        await fastForward(MONTH_SEC);
        expect((await hre.f.c.liquidityPool.lockedCollateral()).quote).to.eq(0);
        assertCloseTo(await getQuoteBalance(hre.f.deployer.address), toBN('990425.7347').div(CONVERTUSDC));
        assertCloseTo(await getQuoteBalance(hre.f.c.shortCollateral.address), partialQuoteCollat.div(CONVERTUSDC));
      });
      ...
      it('sends quote to pool correctly if ITM', async () => {
        ...
        // TODO: double check values with mech
        assertCloseTo(scBaseBal, toBN('5000').div(CONVERTWBTC));
  ...
  describe('Post-Expiry Insolvency: should send total LP profit despite insolvency', async () => {
    ...
    it('short call base', async () => {
      ...
      // TODO: double check values with mech
      assertCloseTo(await getQuoteBalance(hre.f.c.liquidityPool.address), toBN('503412.0666').div(CONVERTUSDC));
    ...
    it('short call quote + short put quote', async () => {
      ...
      // original quote - premium paid + AMM profit
      assertCloseTo(await getQuoteBalance(hre.f.c.liquidityPool.address), toBN('504621.812').div(CONVERTUSDC));
      expect(await hre.f.c.liquidityPool.insolventSettlementAmount()).to.eq(0);
      expect(await hre.f.c.shortCollateral.LPBaseExcess()).to.eq(0);
      expect(await hre.f.c.shortCollateral.LPQuoteExcess()).to.eq(0);
    });
  ...
  describe('Post-Expiry ShortCollateral Depletion: sends less than total LP profit', async () => {
    ...
    it('short call quote', async () => {
      ...
      assertCloseToPercentage(await hre.f.c.shortCollateral.LPQuoteExcess(), insolventAmount.mul(3));
      // original quote - premium paid + AMM profit - insolvency
      // TODO: double check values with mech
      // When the position is settled, the excess is cleared
      await hre.f.c.shortCollateral.settleOptions([positions[0]]);
      assertCloseToPercentage(await hre.f.c.shortCollateral.LPQuoteExcess(), insolventAmount.mul(2));
      await hre.f.c.shortCollateral.settleOptions([positions[1]]);
      assertCloseToPercentage(await hre.f.c.shortCollateral.LPQuoteExcess(), insolventAmount);
      await hre.f.c.shortCollateral.settleOptions([positions[2]]);
      assertCloseToPercentage(await hre.f.c.shortCollateral.LPQuoteExcess(), toBN('0.00000')); // dust left over
    });
