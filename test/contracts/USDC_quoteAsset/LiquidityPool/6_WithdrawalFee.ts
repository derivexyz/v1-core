...
describe('Withdrawal Fees', async () => {
  ...
  // charging fees
  it.skip('charges withdrawal fee, processed when liveBoards != 0', async () => {
    ...
    await hre.f.c.liquidityPool.exchangeBase(); // TestSynthetix.sol hardcoded with quote rate 1e18
    ...
    console.log(`LP bal ${await hre.f.c.liquidityPool.address}`);
  ...
  it('final withdrawal takes all accrued fees when liveBoards == 0', async () => {
    ...
    expect(await hre.f.c.snx.quoteAsset.balanceOf(hre.f.signers[1].address)).to.eq(toBN('105000', 6));
  ...
  it('does not charge withdrawal fee when liveBoards == 0, only long put', async () => {
    ...
    assertCloseToPercentage(newQuote.sub(oldQuote), toBN('500000').add(premium).div(1e12)); //snx fees
  });
});
