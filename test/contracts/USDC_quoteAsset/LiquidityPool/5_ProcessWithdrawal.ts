...
describe('Process withdrawal', async () => {
  beforeEach(async () => {
    ...
    await hre.f.c.snx.quoteAsset.mint(hre.f.alice.address, 100000e6);
    await hre.f.c.snx.quoteAsset.connect(hre.f.alice).approve(hre.f.c.liquidityPool.address, 100000e6);
    await hre.f.c.liquidityPool.connect(hre.f.alice).initiateDeposit(hre.f.alice.address, 10000e6);
  ...
  describe('state tracking', async () => {
    it('tracks withdrawal head, totalQueuedWithdrawal and withdrawal ticket after multiple processes', async () => {
      ...
      assertCloseToPercentage(
        ...
        toBN('100000').sub(totalWithdrawalFee).div(1e12),
        toBN('0.0001'),
      );
      assertCloseToPercentage(
        await hre.f.c.snx.quoteAsset.balanceOf(hre.f.c.liquidityPool.address),
        toBN('500000').add(totalWithdrawalFee).div(1e12),
        toBN('0.0001'),
      );
    });
  ...
  describe('process failures', async () => {
    it('returns if minimumDelay not expired', async () => {
      ...
      await expectBalance(hre.f.c.snx.quoteAsset, toBN('510000', 6), hre.f.c.liquidityPool.address);
    });
  ...
  describe('partial withdrawals', async () => {
    ...
    it('partially withdraws if not enough burnable tokens until fully processed', async () => {
      ...
      expect(await hre.f.c.liquidityPool.queuedWithdrawalHead()).to.eq(1);
      assertCloseTo(newQuote.sub(oldQuote), toBN('50100.25', 6), toBN('5'));
      await validateWithdrawalRecord(
        1,
      ...
      // top off pool
      await hre.f.c.liquidityPool.initiateDeposit(hre.f.deployer.address, 150000e6);
  ...
  describe('edge cases', async () => {
    ...
    it('processess withdrawal after value of pool drops 90%', async () => {
      ...
      await seedBalanceAndApprovalFor(hre.f.deployer, hre.f.c, toBN('100000000', 6), undefined, undefined, true);
      await hre.f.c.liquidityPool.initiateWithdraw(hre.f.alice.address, toBN('100'));
      await hre.f.c.liquidityPool.initiateDeposit(hre.f.alice.address, 100e6);
      ...
      await hre.f.c.liquidityPool.processWithdrawalQueue(2);
      assertCloseToPercentage(await hre.f.c.snx.quoteAsset.balanceOf(hre.f.alice.address), toBN('6.5', 6), toBN('0.1'));
    });
    ...
    it('liveBoards != 0, open options == 0, can withdraw 100%', async () => {
      ...
      // await hre.f.c.liquidityPool.exchangeBase(); // ignored because quote rate hardcoded 18dp in TestSynthetix.sol
      ...
      assertCloseToPercentage(liquidity.freeLiquidity, toBN('4756.376'), toBN('0.01'));
      assertCloseToPercentage(
    ...
    it('allows for settlement after 100% withdrawal', async () => {
      ...
      // confirm settling is fully functional even with 100% withdrawal
      await expectUSDCCorrectSettlement(positionIds);
    });
  });
});
