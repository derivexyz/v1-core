...
describe('USDC_quote - Pool Value', async () => {
  // integration tests
  beforeEach(async () => {
  ...
  describe('base donations', async () => {
    ...
    it('accounts for donated quote in NAV', async () => {
      await hre.f.c.snx.quoteAsset.transfer(hre.f.c.liquidityPool.address, toBN('1000', 6));
    ...
    it.skip('sends less quote to last withdrawer if base is not exchanged - no withdrawal fee', async () => {
      ...
      await hre.f.c.snx.quoteAsset.burn(hre.f.c.liquidityPool.address, DEFAULT_BASE_PRICE.div(1e12));
      await hre.f.c.snx.baseAsset.mint(hre.f.c.liquidityPool.address, toBN('5'));
