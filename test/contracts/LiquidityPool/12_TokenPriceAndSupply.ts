...
// integration tests
describe('TokenPriceAndSupply', async () => {
  ...
  it('gives new depositor accrued fee after 100% withdrawal & liveBoards == 0', async () => {
    ...
    await hre.f.c.snx.quoteAsset.mint(hre.f.signers[1].address, toBN('10000'));
    await hre.f.c.snx.quoteAsset.connect(hre.f.signers[1]).approve(hre.f.c.liquidityPool.address, toBN('10000'));
    await hre.f.c.liquidityPool.connect(hre.f.signers[1]).initiateDeposit(hre.f.signers[1].address, toBN('10000'));
    ...
    expect(await hre.f.c.snx.quoteAsset.balanceOf(hre.f.signers[1].address)).to.eq(toBN('15000'));
  });
