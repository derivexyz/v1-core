...
describe('Liquidity Priority Testing', async () => {
  beforeEach(async () => {
    await seedFixture();
    // console.log(`LP balance ${await hre.f.c.snx.quoteAsset.balanceOf(hre.f.c.liquidityPool.address)}`)
  ...
  describe('Liquidity Priority', async () => {
    it('NAV & LSF remains stable after all insolvent longs close: LSF = 0', async () => {
      ...
      const amountToBurn = (await hre.f.c.snx.quoteAsset.balanceOf(lp)).sub(liquidity.reservedCollatLiquidity);
    ...
    it('NAV stable & LSF rises as all insolvent longs close: LSF < 1', async () => {
      ...
      // console.log("POST TRADE", await hre.f.c.liquidityPool.getLiquidity());
      const lp = hre.f.c.liquidityPool.address;
      const amountToBurn = toBN('250000');
      // console.log("AMOUNT TO BURN", amountToBurn)
      await hre.f.c.snx.quoteAsset.burn(lp, amountToBurn);
    ...
    it('pendingDelta > reservedCollatLiquidity priority', async () => {
      ...
      const [availableQuoteForHedge, afterOpenLiquidity] = await openLongPutAndGetLiquidity(openSize, 3);
      ...
      expect(await hre.f.c.snx.quoteAsset.balanceOf(hre.f.c.liquidityPool.address)).to.eq(
        afterOpenLiquidity.pendingDeltaLiquidity,
      );
    ...
    it('reservedCollatLiquidity > freeLiquidity priority ', async () => {
      ...
      const [availableQuoteForHedge, afterOpenLiquidity] = await openLongCallAndGetLiquidity(openSize);
      ...
      expect(afterOpenLiquidity.freeLiquidity).to.eq(0);
      expect(await hre.f.c.snx.quoteAsset.balanceOf(hre.f.c.liquidityPool.address)).to.eq(
        afterOpenLiquidity.pendingDeltaLiquidity.add(afterOpenLiquidity.reservedCollatLiquidity),
      );
