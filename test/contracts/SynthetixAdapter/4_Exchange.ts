...
describe('Exchange', async () => {
  ...
  beforeEach(async () => {
    ...
    await hre.f.c.snx.quoteAsset.approve(hre.f.c.synthetixAdapter.address, MAX_UINT);
    await hre.f.c.snx.baseAsset.approve(hre.f.c.synthetixAdapter.address, MAX_UINT);
  ...
  describe('zero return', async () => {
    it('return zero if base is zero', async () => {
      ...
      await hre.f.c.synthetixAdapter.exchangeToExactBase(hre.f.c.optionMarket.address, 0);
    ...
    it('quote > 1e10: reverts if base received is zero', async () => {
      ...
      await expect(
        hre.f.c.synthetixAdapter.exchangeToExactBase(hre.f.c.optionMarket.address, toBN('1')),
    ...
    it('0 < base < 1e10: does not revert if quote received is zero', async () => {
      ...
      await hre.f.c.synthetixAdapter.exchangeToExactBase(hre.f.c.optionMarket.address, 1e9 / 2000);
      await expectBalance(hre.f.c.snx.quoteAsset, oldQuoteBalance);
      await expectBalance(hre.f.c.snx.baseAsset, oldBaseBalance);
    });
  });
  ...
  describe('exchangeToExactBase', async () => {
    it('reverts if not enough quote', async () => {
      await expect(
        hre.f.c.synthetixAdapter.exchangeToExactBase(hre.f.c.optionMarket.address, oldQuoteBalance.div(1000)),
      ).revertedWith('ERC20: transfer amount exceeds balance');
    });
    it('reverts if invalid base or quote key', async () => {
      await setETHExchangerInvalid();
    ...
    it('exchanges correct amount', async () => {
      await hre.f.c.synthetixAdapter.exchangeToExactBase(hre.f.c.optionMarket.address, toBN('10'));
  ...
  describe('exchangeToExactBaseWithLimit', async () => {
    it('reverts if quote needed > quote limit', async () => {
      await expect(
        hre.f.c.synthetixAdapter.exchangeToExactBaseWithLimit(hre.f.c.optionMarket.address, toBN('1'), toBN('2000')),
  ...
  describe('Estimate', async () => {
    it('spot: $2000, baseFee: 0.3%, quoteFee: 0.6%', async () => {
      ...
      expect(
        await hre.f.c.synthetixAdapter.estimateExchangeToExactBase(hre.f.c.optionMarket.address, toBN('10')),
      ).to.eq(toBN('20060.180541624874624000'));
      expect(
        await hre.f.c.synthetixAdapter.estimateExchangeToExactQuote(hre.f.c.optionMarket.address, toBN('10000')),
      ).to.eq(toBN('5.030181086519114688'));
    });
    it('spot: $1500, baseFee: 0.3%, quoteFee: 0.6%', async () => {
      await setMockParams(toBN('1500'), toBN('0.003'), toBN('0.006'));
      expect(
        await hre.f.c.synthetixAdapter.estimateExchangeToExactBase(hre.f.c.optionMarket.address, toBN('10')),
      ).to.eq(toBN('15045.135406218655968000'));
      expect(
        await hre.f.c.synthetixAdapter.estimateExchangeToExactQuote(hre.f.c.optionMarket.address, toBN('10000')),
      ).to.eq(toBN('6.706908115358819584'));
    });
    it('spot: $1500, baseFee: 1.2%, quoteFee: 2.0%', async () => {
      await setMockParams(toBN('1500'), toBN('0.012'), toBN('0.020'));
      expect(
        await hre.f.c.synthetixAdapter.estimateExchangeToExactBase(hre.f.c.optionMarket.address, toBN('10')),
      ).to.eq(toBN('15182.186234817813765000'));
      expect(
        await hre.f.c.synthetixAdapter.estimateExchangeToExactQuote(hre.f.c.optionMarket.address, toBN('10000')),
      ).to.eq(toBN('6.802721088435374150'));
    });
    it('spot: $2500, baseFee: 0.1%, quoteFee: 0.2%, alternate amounts', async () => {
      await setMockParams(toBN('2500'), toBN('0.001'), toBN('0.002'));
      expect(await hre.f.c.synthetixAdapter.estimateExchangeToExactBase(hre.f.c.optionMarket.address, toBN('1'))).to.eq(
        toBN('2502.502502502502502500'),
      );
      expect(
        await hre.f.c.synthetixAdapter.estimateExchangeToExactQuote(hre.f.c.optionMarket.address, toBN('1000')),
      ).to.eq(toBN('0.400801603206412826'));
    });
  });
  ...
  describe('exchangeToExactQuote', async () => {
    ...
    it('exchanges correct amount', async () => {
      await hre.f.c.synthetixAdapter.exchangeToExactQuote(hre.f.c.optionMarket.address, toBN('1000'));
